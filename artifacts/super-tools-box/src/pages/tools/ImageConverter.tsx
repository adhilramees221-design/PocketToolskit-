  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {preview && (
          <>
            <img ref={imgRef} src={preview} alt="preview" className="max-h-48 rounded-xl mx-auto border shadow-sm" />
            <p className="text-center text-sm text-muted-foreground">
              {file?.name} · {origSizeKb} KB · {imgRef.current?.naturalWidth}×{imgRef.current?.naturalHeight}px
            </p>
          </>
        )}
        ...
        <div className="grid grid-cols-3 gap-3">
          {FORMATS.map((fmt) => (
            <Button key={fmt.ext} variant="outline" disabled={!file} onClick={() => convert(fmt)} className="gap-2 h-14 flex-col text-xs">
