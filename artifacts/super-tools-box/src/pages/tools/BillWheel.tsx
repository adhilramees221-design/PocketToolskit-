...
const SCENARIOS = [
  ...
  { label: "Who Buys Tea/Coffee? ☕", template: (n: string) => `${n} buys tea/coffee today! ☕🎉` },
...
export default function BillWheel() {
  ...
  const spin = () => {
    if (nameList.length < 2 || spinning) return;
  ...
  const onSpinComplete = () => {
    if (targetIndex !== null) {
      setResult(SCENARIOS[scenario].template(nameList[targetIndex]));
    }
    setSpinning(false);
  };
  ...
  const shareWA = () => {
    ...
    window.open(
      `https://api.whatsapp.com/send?text=${encodeURIComponent(
        `${result}\n\nDecided fairly at pockettoolskit.com 🎡`
      )}`,
      "_blank"
    );
  };
    ...
    <ToolLayout
      ...
      instructions={
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>Choose a scenario, then spin to pick randomly — no arguments!</li>
    ...
    >
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          <Label htmlFor="names">Names (comma-separated, min 2)</Label>
          <Input
            id="names"
            placeholder="Arun, Rahul, Priya, Mohammed, Lakshmi"
            value={names}
            onChange={(e) => {
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {SCENARIOS.map((s, i) => (
              <button
                key={i}
                onClick={() => { setScenario(i); setResult(null); }}
        ...
        {/* Wheel — only shown when names are ready */}
        {nameList.length >= 2 && (
          <div className="py-1">
            <SpinWheel
              items={nameList}
        ...
        <div className="text-center">
          <Button
            size="lg"
            onClick={spin}
            disabled={spinning || nameList.length < 2}
